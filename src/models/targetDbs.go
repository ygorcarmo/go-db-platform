...
func (targetDb *TargetDb) GetByName(name string) (*TargetDb, error) {
	err := db.Database.QueryRow("SELECT * FROM db_connection_info WHERE name=?;", name).Scan(&targetDb.Name, &targetDb.Host, &targetDb.Port, &targetDb.Type, &targetDb.SslMode)
	return targetDb, err
}
...
func (targetDb *TargetDb) GetAllNames() ([]string, error) {
	...
	rows, err := db.Database.Query("SELECT name FROM db_connection_info")
